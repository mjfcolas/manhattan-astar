...
public class AStarManhattanPathFinder {
    ...
    private final Grid gridContainer;
    final AStarNode[][] copiedGrid;

    public AStarManhattanPathFinder(final Grid gridContainer) {
    ...
    private AStarNode[][] initialize() {
        final Coordinate[][] grid = gridContainer.getCells();
        final AStarNode[][] copiedGrid = new AStarNode[width][height];
        for (int x = 0; x < width; x++) {
            for (int y = 0; y < height; y++) {
                copiedGrid[x][y] = new AStarNode(grid[x][y]);
            }
        }
        return copiedGrid;
    }
